...
function Swap({ selectedProvider, tokenListURI }) {
  ...
  const getAccountInfo = async () => {
    if (tokens) {
      ...
      if (tokenOut) {
        ...
        const newBalanceOut = await getBalance(
          tokenOut,
          accountList[0],
          tempContractOut
        )
  ...
  const insufficientBalance = balanceIn
    ? parseFloat(
        ethers.utils.formatUnits(balanceIn, tokens[tokenIn].decimals)
      ) < amountIn
    : null
  ...
  const formattedBalanceOut = balanceOut
    ? parseFloat(
        ethers.utils.formatUnits(balanceOut, tokens[tokenOut].decimals)
      ).toPrecision(6)
    : null
    ...
    >
      <Space direction='vertical'>
        <Row justify='center' align='middle'>
          <Card
            ...
            type='inner'
            title={`From${
              exact === 'out' && tokenIn && tokenOut ? ' (estimate)' : ''
            }`}
            extra={
              <>
                <img src={logoIn} alt={tokenIn} width='30' />
        ...
        <Row justify='center' align='middle'>
          <Card
            ...
            type='inner'
            title={`To${
              exact === 'in' && tokenIn && tokenOut ? ' (estimate)' : ''
            }`}
            extra={
              <>
      ...
      >
        <Descriptions title='Details' column={1} style={{ textAlign: 'left' }}>
          <Descriptions.Item label='blockNumber'>
            {blockNumber}
          </Descriptions.Item>
          <Descriptions.Item label='routerAllowance'>
            <Space>
              {routerAllowance
                ...
                : null}
              {routerAllowance > 0 ? (
                <Button onClick={removeRouterAllowance}>
                  Remove Allowance
                </Button>
              ) : null}
            </Space>
